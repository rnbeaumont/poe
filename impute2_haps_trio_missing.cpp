...
#include "impute2_haps_trio_missing.h"
	...
	for(int j=0;j<loopnum;++j){
		...
		for(int i=0;i<cInds;i++){
			...
			if(!(mhash.find(cids[i])==mhash.end()) && !(fhash.find(cids[i])==fhash.end())){
				...
				//find mother index
				mind=mhash[cids[i]];
				compare_pair_genotypes(cHapArray,mHapArray,j,i,mind,outpat,outmat,outhet,transmitted,loopnum);
			}else if(!(fhash.find(cids[i])==fhash.end())){
				//find father index
				find=fhash[cids[i]];
				compare_pair_genotypes(cHapArray,fHapArray,j,i,find,outmat,outpat,outhet,transmitted,loopnum);
			}else{
				outpat<<" 0 0 0";
				outmat<<" 0 0 0";
				outhet<<" 0 0 0";
...
void compare_pair_genotypes(vector<vector<float>>& cHapArray,vector<vector<float>>& mHapArray,int j,int i,int mind,filtering_ostream& outpat,filtering_ostream& outmat,filtering_ostream& outhet,bool transmitted,int loopnum){
	if(mHapArray[j][2*mind]<0.5 && mHapArray[j][2*mind+1]<0.5){ //AA
		if(cHapArray[j][2*i]<0.5 && cHapArray[j][2*i+1]<0.5){ // AA
			outpat<<" 1 0 0";
			outmat<<" 1 0 0";
